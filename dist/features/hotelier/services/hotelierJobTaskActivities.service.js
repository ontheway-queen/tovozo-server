...
const customError_1 = __importDefault(
	require("../../../utils/lib/customError")
);
...
class HotelierJobTaskActivitiesService extends abstract_service_1.default {
	constructor() {
		super();
		this.approveJobTaskActivity = (req) =>
			__awaiter(this, void 0, void 0, function* () {
				const id = req.params.id;
				const { user_id } = req.hotelier;
				return yield this.db.transaction((trx) =>
					__awaiter(this, void 0, void 0, function* () {
						const userModel = this.Model.UserModel(trx);
						const jobPostModel = this.Model.jobPostModel(trx);
						const jobApplicationModel =
							this.Model.jobApplicationModel(trx);
						...
						const taskActivity =
							yield jobTaskActivitiesModel.getSingleTaskActivity({
								id: Number(id),
							});
						if (
							taskActivity.application_status !==
							constants_1.JOB_APPLICATION_STATUS
								.WaitingForApproval
						) {
							throw new customError_1.default(
						...
						const application =
							yield jobApplicationModel.getMyJobApplication({
								job_application_id:
						...
						const res =
							yield jobTaskActivitiesModel.updateJobTaskActivity(
								taskActivity.id,
								{
						...
						yield jobPostModel.updateJobPostDetailsStatus({
							...
							status: constants_1.JOB_POST_DETAILS_STATUS
								.In_Progress,
						});
						...
						yield this.insertNotification(
							trx,
							userModelTypes_1.TypeUser.JOB_SEEKER,
							{
								...
								related_id: res[0].id,
								type: commonModelTypes_1.NotificationTypeEnum
									.JOB_TASK,
							}
						);
						const isJobSeekerOnline = yield (0,
						socket_1.getAllOnlineSocketIds)({
							user_id: taskActivity.job_seeker_id,
							type: userModelTypes_1.TypeUser.JOB_SEEKER,
						});
						if (isJobSeekerOnline && isJobSeekerOnline.length > 0) {
							socket_1.io
								.to(String(taskActivity.job_seeker_id))
								.emit(
									commonModelTypes_1.TypeEmitNotificationEnum
										.JOB_SEEKER_NEW_NOTIFICATION,
									{
										user_id: taskActivity.job_seeker_id,
										photo: hotelier[0].photo,
										title: this.NotificationMsg.JOB_ASSIGNED
											.title,
										content:
											this.NotificationMsg.JOB_ASSIGNED.content(
												{
													id: application.job_post_details_id,
													jobTitle:
														application.job_post_title,
												}
											),
										related_id: res[0].id,
										type: commonModelTypes_1
											.NotificationTypeEnum.JOB_TASK,
										read_status: false,
			...
			__awaiter(this, void 0, void 0, function* () {
				...
				return yield this.db.transaction((trx) =>
					__awaiter(this, void 0, void 0, function* () {
						...
						const jobTaskActivitiesModel =
							this.Model.jobTaskActivitiesModel(trx);
						const jobTaskListModel =
							this.Model.jobTaskListModel(trx);
						const hotelier = yield userModel.checkUser({
							id: user_id,
							type: userModelTypes_1.TypeUser.HOTELIER,
						});
						...
						const res = yield jobTaskListModel.createJobTaskList(
							taskList
						);
						if (!res.length) {
							throw new customError_1.default(
						...
						yield jobApplicationModel.updateMyJobApplicationStatus({
							application_id: taskActivity.job_application_id,
							job_seeker_id: taskActivity.job_seeker_id,
							status: constants_1.JOB_APPLICATION_STATUS
								.IN_PROGRESS,
						});
						const isJobSeekerExists = yield userModel.checkUser({
							id: taskActivity.job_seeker_id,
						...
						const allMessages = taskList
							.map(
								(task, index) => `${index + 1}. ${task.message}`
							)
							.join("\n");
						yield this.insertNotification(
							trx,
							userModelTypes_1.TypeUser.JOB_SEEKER,
							{
								...
								sender_id: user_id,
								sender_type: constants_1.USER_TYPE.HOTELIER,
								title: this.NotificationMsg.NEW_TASKS_ASSIGNED
									.title,
								content: allMessages,
						...
						if (isJobSeekerOnline && isJobSeekerOnline.length > 0) {
							...
							if (isJobSeekerExists[0].device_id) {
								yield lib_1.default.sendNotificationToMobile({
									to: isJobSeekerExists[0].device_id,
									notificationTitle:
										this.NotificationMsg.NEW_TASKS_ASSIGNED
											.title,
		...
		this.approveEndJobTaskActivity = (req) =>
			__awaiter(this, void 0, void 0, function* () {
				return yield this.db.transaction((trx) =>
					__awaiter(this, void 0, void 0, function* () {
						const id = req.params.id;
						...
						const jobPost =
							yield jobPostModel.getSingleJobPostForAdmin(
						...
						const lastPaymentId =
							yield paymentModel.getLastPaymentId();
						const payId =
							lastPaymentId &&
							(lastPaymentId === null || lastPaymentId === void 0
								? void 0
						...
						const paymentRes = yield paymentModel.initializePayment(
							paymentPayload
						);
						const res =
							yield jobTaskActivitiesModel.updateJobTaskActivity(
						...
						console.log({ jobPost });
						yield cancelHotelierJobsIfUnpaidQueue.add(
							"cancelHotelierJobsIfUnpaid",
							{
								id: jobPost.id,
						...
						yield this.insertNotification(
							...
							userModelTypes_1.TypeUser.JOB_SEEKER,
							{
								...
								sender_id: user_id,
								sender_type: constants_1.USER_TYPE.HOTELIER,
								title: this.NotificationMsg.TASK_UNDER_REVIEW
									.title,
								content:
						...
						if (isJobSeekerOnline && isJobSeekerOnline.length > 0) {
							...
							if (isJobSeekerExists[0].device_id) {
								yield lib_1.default.sendNotificationToMobile({
									to: isJobSeekerExists[0].device_id,
									notificationTitle:
										this.NotificationMsg.TASK_UNDER_REVIEW
											.title,
									notificationBody:
										this.NotificationMsg.TASK_UNDER_REVIEW.content(
											application.job_post_details_id
