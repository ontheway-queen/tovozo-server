    ...
    getAllAdmin(query_1) {
        return __awaiter(this, arguments, void 0, function* (query, is_total = false) {
            ...
            const data = yield this.db("admin as ua")
                .withSchema(this.ADMIN_SCHEMA)
                .select("ua.user_id", "u.username", "u.name", "u.email", "u.phone_number", "u.photo", "u.status", "rl.name as role", "rl.id as role_id", "ua.is_2fa_on", "ua.is_main")
    ...
    getSingleAdmin(payload) {
        return __awaiter(this, void 0, void 0, function* () {
            return yield this.db("admin as ua")
                .select("ua.*", "u.username", "u.name", "u.email", "u.phone_number", "u.photo", "u.password_hash", "u.status", "rl.name as role", "rl.id as role_id")
                .withSchema(this.ADMIN_SCHEMA)
                .leftJoin("roles as rl", "rl.id", "ua.role_id")
                .joinRaw("LEFT JOIN dbo.user u ON u.id = ua.user_id")
                .where((qb) => {
