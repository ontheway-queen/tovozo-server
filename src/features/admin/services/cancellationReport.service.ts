  ...
  public async updateCancellationReportStatus(req: Request) {
    return await this.db.transaction(async (trx) => {
      ...
      if (body.status === CANCELLATION_REPORT_STATUS.APPROVED) {
        await reportModel.updateCancellationReportStatus(Number(id), body);
        if (report_type === REPORT_TYPE.CANCEL_JOB_POST) {
          const jobPost = await jobPostModel.getSingleJobPost(report.id);
          ...
          const application = await jobApplicationModel.cancelMyJobApplication(
            report.related_id,
            report.reporter_id
          );
          await jobPostModel.updateJobPostDetailsStatus(
