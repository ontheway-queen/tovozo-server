...
class AdminAdministrationController extends AbstractController {
	...
	public getSingleRolePermission = this.asyncWrapper.wrap(
		...
		async (req: Request, res: Response) => {
			const { code, ...data } =
				await this.service.getSingleRolePermission(req);
			res.status(code).json(data);
		}
	);
	...
	public updateRolePermissions = this.asyncWrapper.wrap(
		...
		async (req: Request, res: Response) => {
			const { code, ...data } = await this.service.updateRolePermissions(
				req
			);
			res.status(code).json(data);
		}
	...
	//update admin
	public getAllAuditTrails = this.asyncWrapper.wrap(
		null,
		async (req: Request, res: Response) => {
			const { code, ...data } = await this.service.getAllAuditTrails(req);
			res.status(code).json(data);
		}
	);
}
