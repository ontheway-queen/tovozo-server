...
class PublicController extends AbstractController {
	...
	public sendEmailOtpController = this.asyncWrapper.wrap(
		...
		async (req: Request, res: Response) => {
			const { code, ...rest } = await this.services.sendOtpToEmailService(
				req
			);
	...
	public matchEmailOtpController = this.asyncWrapper.wrap(
		...
		async (req: Request, res: Response) => {
			const { code, ...data } = await this.services.matchEmailOtpService(
				req
			);
	...
	public getAllNationality = this.asyncWrapper.wrap(
		...
		async (req: Request, res: Response) => {
			const { code, ...data } = await this.services.getAllNationality(
				req
			);
			res.status(code).json(data);
		}
