import {
  APP_STORE_ICON,
  JOB_SEEKER_APP_STORE_URL,
  JOB_SEEKER_PLAY_STORE_URL,
  PLAY_STORE_ICON,
  PROJECT_LOGO,
  PROJECT_NAME,
  PROJECT_NUMBER,
...
export const registrationJobSeekerTemplate = (payload: { name: string }) => {
  return `
    <!DOCTYPE html>
<html lang="en">
<head>
  ...
  <div style="max-width:450px; margin:20px auto; background:#fff; border-radius:8px; overflow:hidden; box-shadow:0 4px 10px rgba(0,0,0,0.1); border:1px solid #e7e9ec;">
    ...
    <div style="padding:20px;">
      ...
      <p style="margin:10px 0; font-size:14px; line-height:1.5;">
        Thank you for registering with <strong>${PROJECT_NAME}</strong>. We will review your information shortly.
      </p>
      <p style="margin:10px 0; font-size:14px; line-height:1.5;">
    ...
    <!-- Footer -->
        <div style="background-color:#f1f5fa; text-align:center; padding:10px; font-size:12px; color:#666;">
          ...
          <div style="margin:10px 0;">
            <a href="${JOB_SEEKER_APP_STORE_URL}" target="_blank" style="display:inline-block; margin:0 5px;">
              <img src="${APP_STORE_ICON}" alt="Download on the App Store" style="height:30px;" />
            </a>
            <a href="${JOB_SEEKER_PLAY_STORE_URL}" target="_blank" style="display:inline-block; margin:0 5px;">
